...
- **Synergy with D-Level Suite**: Serves as the primary autonomous execution engine of the suite, orchestrating multi-ticket batches with automated circuit breakers, test feedback, and notification dispatches.
...
- **Sub-task Notification**: Upon completing each task, trigger notification if hook exists:
  ```bash
  node .d-level/scripts/notify.js --event=task_done --title="Completed: [Task Title]" --message="Passed validation tests"
  ```
- **Circuit Breaker**: If the agent attempts 5 consecutive self-corrections on the exact same error without progress, pause, record the blocker, and trigger:
  ```bash
  node .d-level/scripts/notify.js --event=circuit_breaker --title="Blocked: [Task Title]" --message="Stuck on error after 5 retries" --details="[Error snippet]"
  ```
  Then write the blocker to `.d-level/tuning-log.md` and alert the developer.

### Step 3: Completion Report
When all validation checks pass:
1. **Trigger Batch Completion Notification**:
   ```bash
   node .d-level/scripts/notify.js --event=batch_done --title="All Tasks Completed" --message="Completed all [N] tasks successfully"
   ```
